...
import com.sean.service.constant._RP;
...
public final class ActionContainer
{
	...
	public ActionContainer(List<Class<?>> actCls, List<Class<?>> paramCls, List<Class<?>> retParamCls, FrameworkSpi userInterface,
			InterceptorInvoker interceptorInvoker, BeanContainer beanContainer) throws Exception
	{
		// 日志
		...
		Map<String, ParameterEntity> params = new HashMap<String, ParameterEntity>(512);
		...
		Map<String, ReturnParameterEntity> retparams = new HashMap<String, ReturnParameterEntity>(512);
		...
		for (int i = 0; i < retParamCls.size(); i++)
		{
			...
			for (int j = 0; j < fs.length; j++)
			{
				...
				if (rpc != null)
				{
					ReturnParameterEntity param = returnParamParser.parse(rpc, f.get(obj).toString());
		...
		{
			// 初始化Action
			ActionEntity ae = actionParser.parse(actCls.get(i), params, retparams);
			...
			// 加入Action容器，并打印日志
			actlist.add(act);
			logger.info("Action container load action " + ae.getCls().getName() + " successfully, version is " + ae.getVersion().getVersion());
			...
			for (Field f : fs)
			{
				...
				{
					Object bean = beanContainer.getBean(f.getType().getName());
					f.setAccessible(true);
					f.set(act, bean);
					logger.info("inject bean into field " + f.getName() + " in the action " + act.getClass().getName() + ", version is "
