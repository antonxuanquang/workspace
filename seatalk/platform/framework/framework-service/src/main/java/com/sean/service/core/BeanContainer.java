...
import com.sean.service.parser.BeanParser;
	...
	private void createBean(BeanEntity be) throws Exception
	{
		...
		if (bean == null)
		{
			...
			for (int i = 0; i < inters.length; i++)
			{
				...
				// 打印创建日志
				logger.info("create bean " + be.getCls().getName() + " to BeanContainer named " + inters[i].getName());
			}
			this.beans.put(be.getCls().getName(), bean);
			// 打印创建日志
			logger.info("create bean " + be.getCls().getName() + " to BeanContainer named " + be.getCls().getName());
		}
	}
		...
		for (int i = 0; i < fs.length; i++)
		{
			...
			if (!Modifier.isStatic(f.getModifiers()) && !Modifier.isFinal(f.getModifiers()))
			{
				...
				if (rc != null)
				{
					...
					{
						f.set(bean, res);
						logger.info("inject " + res.getClass().getName() + " into field " + f.getName() + " in the bean " + beanName);
					}
					catch (Exception e)
					{
						e.printStackTrace();
