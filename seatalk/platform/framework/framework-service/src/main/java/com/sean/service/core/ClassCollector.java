...
import com.sean.commom.util.ClassScanner;
...
public final class ClassCollector
{
	...
	public Map<String, List<Class<?>>> collect() throws Exception
	{
		...
		List<Class<?>> beans = new ArrayList<Class<?>>(20);
	...
	private void list(Map<String, List<Class<?>>> classes) throws Exception
	{
		...
		for (int i = 0; i < length; i++)
		{
			...
			if (cls != null)
			{
				...
				// 如果是Bean
				else if (cls.getAnnotation(BeanConfig.class) != null)
				{
					classes.get("beans").add(cls);
				}
