...
public class FieldWriterEntity extends FieldWriter
{
	...
	public void write(StringBuilder json, Map<String, Object> retMap, ReturnParameterEntity param) throws Exception
	{
		...
		if (entity != null)
		{
			...
			// 获取所有数据字典值
			Map<String, String> dicVals = new HashMap<String, String>();
			UseDicEntity[] dics = param.getDics();
			UseDicEntity dic;
			for (int i = 0; i < dics.length; i++)
			{
				dic = dics[i];
				PersistContext.CTX.getDictionary(dic.getDic()).getDicVal(Long.parseLong(data.get(dic.getField()).toString()), dicVals);
			}
